...
function normalizeProduct(p) {
  return {
    ...
    images: p.images ?? [], // ✅ đọc từ MockAPI
  };
}
...
function renderProducts(list) {
  ...
  $$(".btn-detail").forEach(
    (btn) => (btn.onclick = () => openDetail(btn.dataset.id))
  );
}
...
function openDetail(id) {
  ...
  const imgs = [p.img, ...(p.images || [])]; // ảnh chính + phụ
  mainImg.src = imgs[0];
